...
@section('content_header')
    <h1>{{ __('Edit Medical Record') }}</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('Update Medical Record') }}</h3>
            ...
            <div class="card-body">
                    ...
                    <select name="doctor_id" id="doctor_id"
                        ...
                        @foreach ($doctors as $doctor)
                            <option value="{{ $doctor->id }}"
                                {{ old('doctor_id', default: $medicalRecord->doctor_id) == $doctor->id ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <textarea name="diagnosis" id="diagnosis"
                              class="form-control @error('diagnosis') is-invalid @enderror">{{ old('diagnosis', $medicalRecord->diagnosis) }}</textarea>
                    @error('diagnosis')
                        <span class="invalid-feedback d-block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="treatment" id="treatment"
                              class="form-control @error('treatment') is-invalid @enderror">{{ old('treatment', $medicalRecord->treatment) }}</textarea>
                    @error('treatment')
                        <span class="invalid-feedback d-block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="prescription" id="prescription"
                              class="form-control @error('prescription') is-invalid @enderror">{{ old('prescription', $medicalRecord->prescription) }}</textarea>
                    @error('prescription')
                        <span class="invalid-feedback d-block">{{ $message }}</span>
                    @enderror
                </div>
